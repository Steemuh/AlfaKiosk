...
// This function should be provided by the consuming app
// For server-side auth, pass the auth client in headers
type AuthClient = { 
	getAccessToken: () => Promise<string>;
	fetchWithAuth: (url: string, init: RequestInit) => Promise<Response>;
};
type GetAuthClient = () => Promise<AuthClient>;

// @ts-ignore - used later in executeGraphQL
let authClientGetter: GetAuthClient | null = null;

export function setAuthClientGetter(getter: GetAuthClient) {
	authClientGetter = getter;
}
...
export async function executeGraphQL<Result, Variables>(
	...
	const response = withAuth && authClientGetter
		? await (await authClientGetter()).fetchWithAuth(process.env.NEXT_PUBLIC_SALEOR_API_URL, input)
